#!/usr/bin/env python3
"""Group student turns into teacher-prompt / student-response exchanges.

A "student turn" in this PoC is a rough intermediate unit produced by
`extract_student_turns.py`.

An "exchange" here means:
    - one teacher-side prompt or context
    - one student-side response that may span multiple adjacent student turns
    - ambiguous student-turn boundaries are resolved with a mix of rules and
      LLM judgments

This exchange unit is intended to be the final review unit.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Group student turns into teacher-prompt / student-response exchanges.",
    )
    parser.add_argument(
        "input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional output JSON path. Defaults to <input>.student_exchanges.json",
    )
...
def default_output_path(input_path: Path) -> Path:
    ...
    if input_path.name.endswith(suffix):
        return input_path.with_name(
            input_path.name.removesuffix(suffix) + ".student_exchanges.json"
        )
    return input_path.with_name(input_path.stem + ".student_exchanges.json")
...
def classify_boundaries(turns: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for index in range(len(turns) - 1):
        ...
        boundary_kind = (
            "same_teacher_prompt"
            if same_prompt
            else ("short_teacher_reaction" if short_reaction else "hard_split")
        )
...
def infer_boundary_decisions(candidates: list[dict], api_key: str) -> list[dict]:
        ...
        boundary_sections.append(
            "\n".join(
                [
                    ...
                    f"Shared or latest teacher prompt: {candidate['shared_prompt'] or '(none)'}",
    ...
    system_prompt = (
        "You are grouping adjacent student turns from an English speaking lesson into larger "
        "teacher-prompt / student-response exchanges. "
        "Each boundary is already pre-filtered as ambiguous. "
        "Decide MERGE only if the right turn is clearly a continuation of the same student "
        "response to the same teacher prompt. "
        "Keep split if the student has started a new answer, restarted after a meaningful pause, "
        "or if the short teacher reaction meaningfully changed the flow. "
        "Use semantic continuity, unfinished phrasing, discourse markers, and whether the teacher "
        "text looks like a mere acknowledgment. "
...
def build_exchanges(turns: list[dict], boundary_decisions: list[dict]) -> list[dict]:
    ...
    current = {
        ...
        "student_response_text": turns[0]["text"],
        "teacher_prompt_text": turns[0].get("prev_teacher_text"),
        "teacher_prompt_start": turns[0].get("prev_teacher_start"),
        "teacher_prompt_end": turns[0].get("prev_teacher_end"),
    }
    ...
    for index in range(len(turns) - 1):
        ...
        if should_merge:
            ...
            current["student_response_text"] = stitch_text(
                current["student_response_text"],
                right["text"],
            )
        ...
        exchange_id = f"student_exchange_{len(exchanges) + 1:03d}"
        exchanges.append(
            {
                ...
                "teacher_prompt_text": current["teacher_prompt_text"],
                "teacher_prompt_start": current["teacher_prompt_start"],
                "teacher_prompt_end": current["teacher_prompt_end"],
                "student_response_text": current["student_response_text"],
        ...
        current = {
            ...
            "student_response_text": right["text"],
            "teacher_prompt_text": right.get("prev_teacher_text"),
            "teacher_prompt_start": right.get("prev_teacher_start"),
            "teacher_prompt_end": right.get("prev_teacher_end"),
        }
...
def main() -> int:
    ...
    exchanges = build_exchanges(turns, boundary_decisions)
    ...
    print(f"Exchanges: {len(exchanges)}", flush=True)
