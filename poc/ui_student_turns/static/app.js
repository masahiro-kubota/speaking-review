...
function updateSummary() {
  if (!speechData) {
    ...
    itemCountLabel.textContent = "Exchanges";
  ...
  if (status === "not_reviewed") {
    return `
      <section class="review-block review-pending">
        ...
        <p class="section-text">この exchange にはまだ添削結果がありません。</p>
      </section>
    `;
  }
...
function renderItems() {
  if (!speechData || speechData.items.length === 0) {
    turnList.innerHTML = `<article class="empty-card">表示できる exchange がありません。</article>`;
    updateSummary();
    return;
  }
  ...
  const cards = speechData.items
    .map((item) => {
      ...
      const promptText = item.prompt_text || "Prompt がありません。";
      const promptAvailable = Boolean(item.prompt_text && item.prompt_start !== null);
      const sourceUnitMeta = `<span>Source turns: ${item.source_unit_count}</span>`;
      return `
        <article class="turn-card ${isPlaying ? "card-playing" : ""}" data-item-id="${item.id}">
          ...
          <div class="turn-sections">
            ...
            <section class="turn-section student-section">
              ...
              <p class="section-text">${escapeHtml(item.student_text || "-")}</p>
            </section>
          </div>
...
function playItem(itemId, withPrompt) {
  ...
  const promptStartAvailable =
    withPrompt &&
    item.prompt_start !== null &&
    Number.isFinite(Number(item.prompt_start)) &&
    Number(item.prompt_start) < item.end;
  const startAt = promptStartAvailable ? Number(item.prompt_start) : item.start;
  startPlayback(startAt, item.end, item.id);
}

async function loadStudentSpeech(lesson) {
  const response = await fetch(`/api/student-speech?lesson=${encodeURIComponent(lesson)}`);
  if (!response.ok) {
    turnList.innerHTML = `<article class="empty-card">Student exchange を読み込めませんでした。</article>`;
    return;
  }

  speechData = await response.json();
  audioPlayer.src = `/api/audio?lesson=${encodeURIComponent(speechData.name)}`;
  currentStopAt = null;
  currentItemId = null;
  renderItems();
...
async function loadStudentExchangeList() {
  ...
  if (items.length === 0) {
    ...
    turnList.innerHTML = `<article class="empty-card">poc/output/&lt;lesson&gt;/merged.student_exchanges.json がありません。</article>`;
    speechData = null;
    updateSummary();
    return;
  }
